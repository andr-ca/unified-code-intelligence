...
| 1.7 | Optional LSP/SCIP bridge to promote/prune/discover edges | ✅ | `enrich/base.py` (`EdgeSource`/`EdgeDelta`/`Budget`), `enrich/lsp_client.py`, `enrich/lsp_source.py` — all three modes: **Verify** (promote→`lsp-verified`/prune-with-tombstone), **Discover** (`unresolved_calls`→definition→new edges), **Complete** (references→`REFERENCES`); `enrich/scip_source.py` (`resolution="scip"`), `enrich/servers.py` (Che4z COBOL + pyright + tsserver). `engine.enrich_edges` + `uci enrich --lsp/--scip [--verify-only|--complete]`. **Eval:** `evals/lsp_eval.py` scores promote/prune/discover/complete/leave precision–recall against a scripted LSP server over real stdio transport (CI-gated, `tests/test_lsp_eval.py`) |
...
**188 tests pass (incl. the full-suite eval baseline gate, `tests/test_evals_gate.py`, and the dashboard: build/eval jobs, multi-project isolation, metrics, graph views, eval authoring + versioning, and config), no Docker/network.**
